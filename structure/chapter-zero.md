...
>>* 插入操作都限定在表尾进行。
>>* 都可以通过顺序结构和链式结构实现。
>>* 插入和删除的时间复杂度为O(1)，在空间复杂度上两者也一样。
>>* 多链栈和多链队列d的管理模式可以相同。
 
> 【栈和队列的不同点】
>>* 队列先进先出，栈先进后出。
>>* 删除数据元素的位置不同，栈的删除操作是在表尾进行，队列的删除操作是在表头进行。
>>* 遍历数据速度不同，栈只能从头部取数据，也就是最先放入的需要遍历整个栈最后才能取出来，而且在遍历数据的时候还得为数据开辟临时空间，保持数据在遍历前后的一致性；队列不同，队列是基于地址指针进行遍历，而且可以从头或者尾部进行遍历，但不能同时遍历，无需开辟临时空间，速度要快得多。
>>* 应用场景不同，常见栈的应用场景包括括号问题的求解，表达式的转换和求值，深度优先搜索；常见的队列的应用场景包括计算机系统中的各种资源的管理，消息缓冲器的管理和广度优先搜索。
...
>【堆、栈、队列】
>>* 栈和队列是两个特殊的线性表，一个先进后出，一个先进先出。
>>* 但是堆不一样：堆是一种经过排序的树形数据结构，每个节点都是一个值；堆的根节点是最大值或者最小值，同时下面的子树也是堆结构。
>>* 栈中分配局部变量空间，只要栈的剩余空间大于申请空间就可以提供内存；堆是向上增长的用于分配程序员申请的内存空间。
>>* 申请效率的对比：栈是系统分配的，堆是程序员申请的，所以前者申请速度较快，后者申请速度较慢
>>* 栈是由操作系统自动分配和释放，存放函数的参数值，局部变量等值，堆一般由程序员分配释放，若程序员不释放，则在程序结束之后由系统释放；堆是在程序执行的时候动态分配内存，栈是在编译时候静态分配内存。
...
>>>* 平衡二叉树：是一种二叉排序树，左右子树高度差绝对值小于等于1，又叫AVL树。
>>>* 哈夫曼树：压缩用的。权值大小排列。
>>>* 完全二叉树：只能从右边为空。
...
>> 【B树】 参考有道云笔记
>>>* 我们之前谈到的树，都是一个节点可以有多个孩子，但是它自身只存储一个元素。二叉树限制很多，节点最多只能有两个孩子；一个节点只能存储一个元素，在元素非常多的时候，就使得要么树的度非常大（节点拥有子树的个数的最大值），要么树的高度非常大，甚至两者都必须足够大才能满足，这就使得内存存区外存次数非常多，这显然成了时间效率上的瓶颈。
>>>* 多路查找树就是为了解决上述问题。
>>>* 多路查找树，其每一个节点的孩子数可以多余两个，且每一个节点处可以存储多个元素。
>>>* 2-3树是这样一种多路查找树：其中的每一个节点都具有两个孩子（我们称它为2节点）或3个孩子（我们称它为三节点）
...
>>>>（2）都是使用贪婪和线性规划，每一步都是选择权值/花费最小的边。 贪婪：一个局部最有解也是全局最优解； 线性规划：主问题包含n个子问题，而且其中有重叠的子问题。
...
>>【排序算法有哪些】 ⭐或者问：C语言总共有多少种排序法
>>>排序算法有很多，每种算法有不同的时间和空间复杂度，效率也有差别，那么针对使用上也有不同的场合。原则上说，数据结构是一门领域，跟语言没有绝对的联系，很多时候同样的算法可以用很多种语言实现。  下面罗列一些常见的算法：插入排序，冒泡排序，选择排序，快速排序，堆排序，归并排序，基数排序，希尔排序等。
...
>>>* 方法1 固定基准元 : 如果输入序列是随机的，处理时间是可以接受的。如果数组已经有序时，此时的分割就是一个非常不好的分割。
>>>* 方法2 随机基准元 : 这是一种相对安全的策略。由于基准元的位置是随机的，那么产生的分割也不会总是会出现劣质的分割。在整个数组数字全相等时，仍然是最坏情况，时间复杂度是O(n^2）
